...
const dbInit = {
	...
	async v3_1DB(c) {
		const restApiColumn = await c.env.db
			.prepare(`SELECT name FROM pragma_table_info('setting') WHERE name = 'rest_api_enabled' LIMIT 1`)
			.first();

		if (!restApiColumn) {
			await c.env.db
				.prepare(`ALTER TABLE setting ADD COLUMN rest_api_enabled INTEGER NOT NULL DEFAULT 1;`)
				.run();
		}
		...
		await c.env.db.batch([
			c.env.db.prepare(`CREATE UNIQUE INDEX IF NOT EXISTS idx_api_key_public_id ON api_key (public_id)`),
			c.env.db.prepare(`CREATE INDEX IF NOT EXISTS idx_api_key_user_status ON api_key (user_id, status)`),
			c.env.db.prepare(`
